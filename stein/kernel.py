...
def stein_discrepancy(theta: Tensor, p_grad: Tensor, sigma: float, delta_t: Tensor, ipfp_iteration: int,
    ...
    dxdkxy = torch.einsum("...bcd,...bc->...bcd", diffs_times_median, kxy)
    dx2d2kxy = torch.einsum("...bcd,...bce->...bcde", diffs, diffs)
    dx2d2kxy = torch.einsum("...,...bcde->...bcde", 1. / h**4, dx2d2kxy)
    dx2d2kxy = torch.einsum("...bcde,...bc->...bcde", dx2d2kxy, kxy)
    dx2d2kxy += torch.einsum("ab,...->...ab", torch.eye(theta.shape[-1], device=theta.device), kxy_times_median)
