...
def stein_discrepancy(theta: Tensor, p_grad: Tensor, sigma: float, delta_t: Tensor, ipfp_iteration: int) -> Tensor:
    ...
    pairwise_dists = torch.cdist(theta.contiguous(), theta.contiguous()) * delta_t * sigma**2
    diffs = (theta.unsqueeze(-2) - theta.unsqueeze(-3)) * delta_t**0.5 * sigma

    indices = torch.triu_indices(theta.shape[-2], theta.shape[-2], 1)
    h = pairwise_dists[..., indices[0], indices[1]].median(dim=-1)[0]
    h = torch.sqrt(
        h / torch.log(torch.tensor(theta.shape[-2] + 1, device=theta.device))).unsqueeze(-1).unsqueeze(-1)
