...
def stein_discrepancy(theta: Tensor, p_grad: Tensor, sigma: float, delta_t: Tensor, ipfp_iteration: int,
    ...
    # h = torch.sqrt(delta_t * h / torch.log(torch.tensor(theta.shape[-2] + 1, device=theta.device))).unsqueeze(-1).unsqueeze(-1)
    h = torch.sqrt(delta_t * h).unsqueeze(-1).unsqueeze(-1)

    kxy = torch.exp(-pairwise_dists / h**2 / 2) / sigma**2# * delta_t # / sigma
