...
def rossler_step_parallel(x: torch.Tensor,
    ...
    # Get shape right for dimension matching with x2_tilde
    size = [1] * (len(t.shape) + 1)
    size[-2] = x_2_tilde_n.shape[-1]
    t = torch.tile(t.unsqueeze(-1), size)
    delta_t = torch.tile(delta_t, size)
