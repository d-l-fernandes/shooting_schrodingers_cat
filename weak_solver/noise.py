...
def rossler_noise_parallel(noise_dims: int, batch_dims: Tuple[int], device) \
    ...
    def generate_noise(delta_t: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        upper_tri = 0.5 * (beta_beta - torch.sqrt(torch.abs(delta_t).unsqueeze(-1)) * gamma.unsqueeze(-1))
        lower_tri = 0.5 * (beta_beta + torch.sqrt(torch.abs(delta_t).unsqueeze(-1)) * gamma.unsqueeze(-2))
        diag = 0.5 * torch.diag_embed(torch.diagonal(beta_beta, dim1=-2, dim2=-1) - torch.abs(delta_t))
